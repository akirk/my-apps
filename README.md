...
For QR-code-friendly shares, link to the site root with `my-apps-install=<app-slug>`. My Apps redirects the request into `/my-apps/`, installs the matching App Store entry if needed, and opens the app's landing page with the remaining query parameters preserved.

Example:

```
https://my.wordpress.net/?my-apps-install=wordcamp-companion&schedule=https%3A%2F%2Fexample.com%2Fschedule.json
```
