(function() {
	...
	function appPublicShareUrlPromise(path, blueprintUrl) {
		var fallback = appShareUrlForPath(path);
		if (!fallback) return Promise.resolve('');

		return appShareBlueprintPromise(path, blueprintUrl).then(function(blueprint) {
			var repos = [];
			(blueprint && blueprint.steps || []).forEach(function(step) {
				if (!step || step.step !== 'installPlugin' || !step.pluginData) return;
				var repo = githubRepoFromUrl(step.pluginData.url);
				if (repo && repos.indexOf(repo) === -1) repos.push(repo);
			});
			if (repos.length !== 1) return fallback;

			return getGithubInfo(repos[0]).then(function(info) {
				if (!info || !info.homepage) return fallback;
				try {
					var url = new URL(info.homepage);
	...
	function renderPluginDetail(pluginPath, plugin, options) {
		...
		shareBtn.href = appStoreDetailUrl(pluginPath, 'plugin');
		...
		shareBtn.addEventListener('click', function(e) {
			...
			e.preventDefault();
			copyTextToClipboard(shareBtn.href).then(function() {
	...
	function renderAppDetail(appPath, app, blueprintUrl, options) {
		...
		var shareUrl = appShareUrlForPath(appPath);
		var shareBtn = document.createElement(shareUrl ? 'a' : 'button');
		if (shareUrl) {
			shareBtn.href = shareUrl;
			appPublicShareUrlPromise(appPath, blueprintUrl).then(function(publicUrl) {
				if (publicUrl) shareBtn.href = publicUrl;
			});
		} else {
			shareBtn.type = 'button';
		}
		shareBtn.className = 'app-detail-share-btn';
		shareBtn.title = shareUrl ? t('copyLink', __( 'Copy link', 'my-apps' )) : 'Copy blueprint';
		shareBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92s2.92-1.31 2.92-2.92-1.31-2.92-2.92-2.92z"/></svg>';
		shareBtn.addEventListener('click', function(e) {
			e.stopPropagation();
			if (shareUrl) {
			...
			appShareBlueprintPromise(appPath, blueprintUrl).then(function(blueprint) {
	...
	var githubCache = {};
	var githubInfoPromises = {};

	function getGithubInfo(repo) {
		if (!isSafeGithubRepo(repo)) return Promise.resolve(null);
		if (githubCache[repo]) return Promise.resolve(githubCache[repo]);
		if (githubInfoPromises[repo]) return githubInfoPromises[repo];

		githubInfoPromises[repo] = fetch('https://api.github.com/repos/' + repo)
			.then(function(res) {
				if (!res.ok) return null;
				return res.json();
			})
			.then(function(data) {
				if (data) githubCache[repo] = data;
				return data;
			})
			.catch(function() {
				return null;
			...
			.then(function(data) {
				delete githubInfoPromises[repo];
				return data;
			});
		return githubInfoPromises[repo];
	}
